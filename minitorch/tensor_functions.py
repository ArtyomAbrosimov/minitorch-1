...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (s,) = ctx.saved_values
        return grad_output * s * (s.zeros(s.shape) + 1.0 - s)
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        inverted_order_list = [0] * len(order_list)
        for i, o in enumerate(order_list):
            inverted_order_list[o] = i
        return grad_output._new(grad_output._tensor.permute(*inverted_order_list)), 0.0
